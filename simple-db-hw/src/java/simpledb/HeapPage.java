...
public class HeapPage implements Page {
	...
	public void deleteTuple(Tuple t) throws DbException {
		...
		if (t.getRecordId().getPageId().equals(pid) && isSlotUsed(t.getRecordId().mTupleID)) {
			...
			for (int i = 0; i < tuples.length; i ++ ) {
				if (tuples[i]!=null && tuples[i].getRecordId().equals(rid)) {
					tuples[i] = null;
				}
			}
		} else {
	...
	public void insertTuple(Tuple t) throws DbException {
		...
		if (getNumEmptySlots() > 0 && t.getTupleDesc().equals(td)) {
			...
			throw new DbException("Attempting to insert Tuple in HeapPage");
	}
	...
	public boolean isSlotUsed(int i) {
		// some code goes here
		int headerIndex = (i / 8); 
		if (headerIndex < 0 || headerIndex >= header.length)
			return false;
		if (((header[headerIndex] >> (i % 8))&1) == 1)
			return true;
		else
			return false;
	}
